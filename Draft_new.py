...
channelModelConfigFolderPath = 'draft config/Channelmodel_parameters.csv'
basicConfigFolderPath = 'draft config/Basic_config_parameters.csv'
UEConfigFolderPath = 'draft config/Configurations_for_UE1.csv'
BSConfigFolderPath = 'draft config/Configurations_for_BS1.csv'
algorithmConfigFolderPath = 'draft config/Algorithm_switch.csv'
################################################################################### 基础参配置 ###########################################################
########################################################################################################################################################
Basic_config_parameters = pands.read_csv(basicConfigFolderPath)   #暂时从根目录的excel中导入模型基本参数
...
#####################BS######################################
P_num = 12928
Configurations_for_BS1 = pands.read_csv(BSConfigFolderPath)   #暂时从根目录的excel中导入模型基本参数
...
Md_BS = Configurations_for_BS1['BS_D_v_lambda'].loc[0]*WaveLength									# BS水平同向振子个数；存储
Nd_BS = Configurations_for_BS1['BS_D_h_lambda'].loc[0]*WaveLength
...
'''
#######################################终端参数###############################################
Configurations_for_UE1 = pands.read_csv(UEConfigFolderPath)   #暂时从根目录的excel中导入模型基本参数
...
Channelmodel_parameters = pands.read_csv(channelModelConfigFolderPath)   #暂时从根目录的excel中导入模型基本参数
...
##########################################################信道模型算法开关#####################################################
algorithm_switch = pands.read_csv(algorithmConfigFolderPath)   #暂时从根目录的excel中导入模型基本参数
...
for bs_index in range(0, BS_Num):
	...
	for ue_index in range(0, UE_Num):
		...
		Doppler_variable = np.zeros((rayNum, clusterNum), dtype=float)
		for Cir_index in range(0, P_num):
			print('Calculating: Cir_index', Cir_index)
			...
			if Trace_UE.lower() == 'Static'.lower():
				...
				Doppler_variable_LOS = Doppler_variable_LOS + np.dot(rx_LOS[Cir_index, :][np.newaxis, :], V_direction_t[Cir_index, :][:, np.newaxis])[0, 0]
				F_rx_V_direction_LOS[Cir_index] = Doppler_variable_LOS
				for u in range(0, U):
				...
				for cluster_index in range(0, clusterNum):
					...
					for ray_index in range(0, rayNum):
						...
						Doppler_variable[ray_index, cluster_index] = Doppler_variable[ray_index, cluster_index] + np.dot(rx[Cir_index, ray_index, cluster_index, :][np.newaxis, :] , V_direction_t[Cir_index, :][:, np.newaxis])[0,0]
						F_rx_V_direction[Cir_index, ray_index, cluster_index] = Doppler_variable[ray_index, cluster_index]
						for u in range(0, U):
				...
				pass      #添加直线变速用运动的坐标运算
				# 添加实时的终端中心点坐标和阵子坐标
				V_direction_t[Cir_index, :] = V_direction + Cir_index * Acc
				if Cir_index == 0:
					UEcenterPosition_t[Cir_index, :] = UEcenterPosition
					MsElementPosition_t[Cir_index, :, :] = MsElementPosition
				else:
					UEcenterPosition_t[Cir_index, :] = UEcenterPosition_t[Cir_index - 1, :] + \
													   (V_direction_t[Cir_index, :] + V_direction_t[Cir_index - 1, :]) * period / 2
					MsElementPosition_t[Cir_index, :] = np.tile(UEcenterPosition_t[Cir_index, :], (U, 1)) + MsElementPosition
				d = np.linalg.norm(UEcenterPosition_t[Cir_index, :] - BScenterPosition)
				delta_x = np.abs(UEcenterPosition_t[Cir_index, 0] - BScenterPosition[0])
				delta_y = np.abs(UEcenterPosition_t[Cir_index, 1] - BScenterPosition[1])
				delta_z = np.abs(UEcenterPosition_t[Cir_index, 2] - BScenterPosition[2])
				theta = math.acos(delta_z / d) if d != 0 else 0
				phi = math.atan(delta_x / delta_y) if delta_y != 0 else 0
				# todo LOS 不符合名称，换一个
				LOS_AOD_tmp = 0
				LOS_ZOD_tmp = 180 - theta
				LOS_AOA_tmp = 0
				LOS_ZOA_tmp = theta
				# calculating the AOA ZOA AOD ZOD angles of LOS path for each period
				if UEcenterPosition_t[Cir_index, 0] >= BScenterPosition[0] and UEcenterPosition_t[Cir_index, 1] <= BScenterPosition[1]:
					LOS_AOA_tmp = 90 + phi
					LOS_AOD_tmp = -(90 - phi)
				elif UEcenterPosition_t[Cir_index, 0] >= BScenterPosition[0] and UEcenterPosition_t[Cir_index, 1] > BScenterPosition[1]:
					LOS_AOA_tmp = -(90 + phi)
					LOS_AOD_tmp = 90 - phi
				elif UEcenterPosition_t[Cir_index, 0] < BScenterPosition[0] and UEcenterPosition_t[Cir_index, 1] <= BScenterPosition[1]:
					LOS_AOA_tmp = 90 - phi
					...
					LOS_AOA_tmp = -(90 - phi)
					LOS_AOD_tmp = 90 + phi
				LOS_AOA_t[Cir_index] = LOS_AOA_tmp
				LOS_AOD_t[Cir_index] = LOS_AOD_tmp
				LOS_ZOA_t[Cir_index] = LOS_ZOA_tmp
				LOS_ZOD_t[Cir_index] = LOS_ZOD_tmp
				LOS_ZOD_d_t[Cir_index] = np.rad2deg(LOS_ZOD_t[Cir_index])
				LOS_AOD_d_t[Cir_index] = np.rad2deg(LOS_AOD_t[Cir_index])
				# calculate NLOS path angles, this angles should sum up the LOS path angle to show mobility
				AOD_t[Cir_index, :, :] = AOD + np.tile(LOS_AOD_tmp, (rayNum, clusterNum))
				AOA_t[Cir_index, :, :] = AOA + np.tile(LOS_AOA_tmp, (rayNum, clusterNum))
				ZOD_t[Cir_index, :, :] = ZOD + np.tile(LOS_ZOD_tmp, (rayNum, clusterNum))
				ZOA_t[Cir_index, :, :] = ZOA + np.tile(LOS_ZOA_tmp, (rayNum, clusterNum))
				ZOD_d_t[Cir_index, :, :] = np.rad2deg(ZOD_t[Cir_index, :, :])
				AOD_d_t[Cir_index, :, :] = np.rad2deg(AOD_t[Cir_index, :, :])
				# calculate the BS antenna gain of LOS path for each period
				A_thetaphiV_LOS[Cir_index] = -min(12 * ((LOS_ZOD_d_t[Cir_index] - 90) / theta_3dB) ** 2, SLA_v)
				A_thetaphiH_LOS[Cir_index] = -min(12 * ((LOS_AOD_d_t[Cir_index]) / phi_3dB) ** 2, A_max)
				A_thetaphi_LOS[Cir_index] = 10 ** (-min(-(A_thetaphiV_LOS[Cir_index] + A_thetaphiH_LOS[Cir_index]),
														A_max) / 10)  # Antenna Gain of Tx
				# calculate  the
				F_tx_LOS_v[:, Cir_index] = np.array([np.sqrt(A_thetaphi_LOS[Cir_index]) * np.cos(np.deg2rad(BS_slant1)),
													 np.sqrt(A_thetaphi_LOS[Cir_index]) * np.sin(
														 np.deg2rad(BS_slant1))])
				F_tx_LOS_h[:, Cir_index] = np.array([np.sqrt(A_thetaphi_LOS[Cir_index]) * np.cos(np.deg2rad(BS_slant2)),
													 np.sqrt(A_thetaphi_LOS[Cir_index]) * np.sin(
														 np.deg2rad(BS_slant2))])

				F_rx_LOS_v[Cir_index, :] = np.array([np.cos(np.deg2rad(UE_slant1)), np.sin(np.deg2rad(UE_slant1))])
				F_rx_LOS_h[Cir_index, :] = np.array([np.cos(np.deg2rad(UE_slant2)), np.sin(np.deg2rad(UE_slant2))])

				F_rx_Polar_matrix_F_tx_LOS_vv[Cir_index] = (F_rx_LOS_v[Cir_index, :] * np.mat([[1, 0], [0, -1]]) * np.expand_dims(F_tx_LOS_v[:, Cir_index], 1))[0, 0]
				F_rx_Polar_matrix_F_tx_LOS_vh[Cir_index] = (F_rx_LOS_v[Cir_index, :] * np.mat([[1, 0], [0, -1]]) * np.expand_dims(F_tx_LOS_h[:, Cir_index], 1))[0, 0]
				F_rx_Polar_matrix_F_tx_LOS_hh[Cir_index] = (F_rx_LOS_h[Cir_index, :] * np.mat([[1, 0], [0, -1]]) * np.expand_dims(F_tx_LOS_h[:, Cir_index], 1))[0, 0]
				F_rx_Polar_matrix_F_tx_LOS_hv[Cir_index] = (F_rx_LOS_h[Cir_index, :] * np.mat([[1, 0], [0, -1]]) * np.expand_dims(F_tx_LOS_v[:, Cir_index], 1))[0, 0]

				tx_LOS[Cir_index, :] = np.array([np.sin(LOS_ZOD_t[Cir_index]) * np.cos(LOS_AOD_t[Cir_index]),
												 np.sin(LOS_ZOD_t[Cir_index]) * np.sin(LOS_AOD_t[Cir_index]),
												 np.cos(LOS_ZOD_t[Cir_index])])  # tx unit vector
				rx_LOS[Cir_index, :] = np.array([np.sin(LOS_ZOA_t[Cir_index]) * np.cos(LOS_AOA_t[Cir_index]),
												 np.sin(LOS_ZOA_t[Cir_index]) * np.sin(LOS_AOA_t[Cir_index]),
												 np.cos(LOS_ZOA_t[Cir_index])])  # rx unit vector
				Doppler_variable = Doppler_variable + np.dot(rx_LOS[Cir_index, :], V_direction_t[Cir_index, :])
				F_rx_V_direction_LOS[Cir_index] = Doppler_variable
				for u in range(0, U):
					rx_du_LOS[Cir_index, u] = np.dot(rx_LOS[Cir_index, :][np.newaxis, :], MsElementPosition_t[Cir_index, u, :][:, np.newaxis])[
						0, 0]
				for cluster_index in range(0, clusterNum):
					for ray_index in range(0, rayNum):
						A_thetaphiV[Cir_index, ray_index, cluster_index] = -min(12 * ((ZOD_d_t[Cir_index, int(
							np.nonzero(MZOD == ray_index)[0]), cluster_index] - 90) / theta_3dB) ** 2, SLA_v)
						A_thetaphiH[Cir_index, ray_index, cluster_index] = -min(12 * ((AOD_d_t[
							Cir_index, int(np.nonzero(MAOD == ray_index)[0]), cluster_index]) / phi_3dB) ** 2,A_max)
						A_thetaphi[Cir_index, ray_index, cluster_index] = 10 ** (-min(-(
									A_thetaphiV[Cir_index, ray_index, cluster_index] + A_thetaphiH[
								Cir_index, ray_index, cluster_index]), A_max) / 10)  # Antenna Gain of Tx
						F_tx_v[:, Cir_index, ray_index, cluster_index] = np.array(
							[np.sqrt(A_thetaphi[Cir_index, ray_index, cluster_index]) * np.cos(
								np.deg2rad(BS_slant1)),
							 np.sqrt(A_thetaphi[Cir_index, ray_index, cluster_index]) * np.sin(
								 np.deg2rad(BS_slant1))])
						F_tx_h[:, Cir_index, ray_index, cluster_index] = np.array(
							[np.sqrt(A_thetaphi[Cir_index, ray_index, cluster_index]) * np.cos(
								np.deg2rad(BS_slant2)),
						...
						F_rx_Polar_matrix_F_tx_vv[Cir_index, ray_index, cluster_index] = (F_rx_v[Cir_index, ray_index, cluster_index, :] * np.mat(
							[[np.exp(1j * InitialPhase_VV[cluster_index, ray_index]),
							  np.sqrt(1 / K) * np.exp(1j * InitialPhase_VH[cluster_index, ray_index])],
							 [np.sqrt(1 / K) * np.exp(1j * InitialPhase_HV[cluster_index, ray_index]),
							  np.exp(1j * InitialPhase_HH[cluster_index, ray_index])]]) * np.expand_dims(F_tx_v[:, Cir_index, ray_index, cluster_index], 1))[0, 0]
						F_rx_Polar_matrix_F_tx_vh[Cir_index, ray_index, cluster_index] = (F_rx_v[Cir_index, ray_index, cluster_index, :] * np.mat(
							[[np.exp(1j * InitialPhase_VV[cluster_index, ray_index]),
							  np.sqrt(1 / K) * np.exp(1j * InitialPhase_VH[cluster_index, ray_index])],
							 [np.sqrt(1 / K) * np.exp(1j * InitialPhase_HV[cluster_index, ray_index]),
							  np.exp(1j * InitialPhase_HH[cluster_index, ray_index])]]) * np.expand_dims(F_tx_h[:, Cir_index, ray_index, cluster_index], 1))[0, 0]
						F_rx_Polar_matrix_F_tx_hh[Cir_index, ray_index, cluster_index] = (F_rx_h[Cir_index, ray_index, cluster_index, :] * np.mat(
							[[np.exp(1j * InitialPhase_VV[cluster_index, ray_index]),
							  np.sqrt(1 / K) * np.exp(1j * InitialPhase_VH[cluster_index, ray_index])],
							 [np.sqrt(1 / K) * np.exp(1j * InitialPhase_HV[cluster_index, ray_index]),
							  np.exp(1j * InitialPhase_HH[cluster_index, ray_index])]]) * np.expand_dims(F_tx_h[:, Cir_index, ray_index, cluster_index], 1))[0, 0]
						F_rx_Polar_matrix_F_tx_hv[Cir_index, ray_index, cluster_index] = (F_rx_h[Cir_index, ray_index, cluster_index, :] * np.mat(
							[[np.exp(1j * InitialPhase_VV[cluster_index, ray_index]),
							  np.sqrt(1 / K) * np.exp(1j * InitialPhase_VH[cluster_index, ray_index])],
							 [np.sqrt(1 / K) * np.exp(1j * InitialPhase_HV[cluster_index, ray_index]),
							  np.exp(1j * InitialPhase_HH[cluster_index, ray_index])]]) * np.expand_dims(F_tx_v[:, Cir_index, ray_index, cluster_index], 1))[0, 0]

						tx[Cir_index, ray_index, cluster_index, :] = np.array([np.sin(
							ZOD_t[Cir_index, int(np.nonzero(MZOD == ray_index)[0]), cluster_index]) * np.cos(
							AOD_t[Cir_index, int(np.nonzero(MAOD == ray_index)[0]), cluster_index]),
																			   np.sin(ZOD_t[Cir_index, int(
																				   np.nonzero(MZOD == ray_index)[
																					   0]), cluster_index]) * np.sin(
																				   AOD_t[Cir_index, int(np.nonzero(
																					   MAOD == ray_index)[0]), cluster_index]),
																			   np.cos(ZOD_t[Cir_index, int(
																				   np.nonzero(MZOD == ray_index)[
																					   0]), cluster_index])])  # tx unit vector
						rx[Cir_index, ray_index, cluster_index, :] = np.array([np.sin(
							ZOA_t[Cir_index, int(np.nonzero(MZOA == ray_index)[0]), cluster_index]) * np.cos(
							AOA_t[Cir_index, int(np.nonzero(MAOA == ray_index)[0]), cluster_index]),
																			   np.sin(ZOA_t[Cir_index, int(
																				   np.nonzero(MZOA == ray_index)[
																					   0]), cluster_index]) * np.sin(
																				   AOA_t[Cir_index, int(np.nonzero(
																					   MAOA == ray_index)[0]), cluster_index]),
																			   np.cos(ZOA_t[Cir_index, int(np.nonzero(MZOA == ray_index)[0]), cluster_index])])  # rx unit vector
						Doppler_variable = Doppler_variable + \
										   np.dot(rx[Cir_index, ray_index, cluster_index, :][np.newaxis, :],
												  V_direction_t[Cir_index, :][:, np.newaxis])[0, 0]
						F_rx_V_direction[Cir_index, ray_index, cluster_index] = Doppler_variable
						for u in range(0, U):
							rx_du[Cir_index, u, ray_index, cluster_index] = \
							np.dot(rx[Cir_index, ray_index, cluster_index, :][np.newaxis, :],
								   MsElementPosition_t[Cir_index, u, :][:, np.newaxis])[0, 0]
...
Channel_verification(delay, period, H_average, LOS_or_NLOS, smallscale_type, M_BS, N_BS, AE_Comb)
