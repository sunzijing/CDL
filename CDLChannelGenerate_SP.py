...
channel_type = 'CDL_C' 					# option：CDL_A/CDL_B/CDL_C/CDL_D/CDL_E(case insensitive)
Cluster_selected = 'OFF'
CentralFrequence = 2.5653*10**9 				# Center Frequency
LightSpeed = 2.99792458*10**8 				# speed of light
WaveLength = LightSpeed / CentralFrequence
k_CONST = 2*np.pi/WaveLength
SD = 2 										# sample density
V = 3 / 3.6  								# velocity m/s
V_direction = V*np.mat((0, 1, 0))            # UE motion Vector

CIR_update_Rate = 2*SD*V*CentralFrequence/LightSpeed # CIR update rate
#CIR_update_Rate = 2*SD*3/3.6*CentralFrequence/LightSpeed # CIR update rate
S = 12*8 									# CE input antenna element number
U = 2  									# CE output antenna element number
UEPolarizationPlacement = 'UEVerticalCross'	 # Vertical Cross +；Cross X；Pure Vertical |；Pure Horizontal —
BSPolarizationPlacement = 'BSPureVeical' 			# Vertical Cross +；Cross X；Pure Vertical |；Pure Horizontal —
...
Mpu = 1										# #Ms垂直同向振子个数；
Npu = 1										# #Ms水平同向振子个数；
Md = 0.7*WaveLength							# #垂直振子间距；
Nd = 0.5*WaveLength							# #水平振子间距；
...
P_num = 12000							#修改时间长度，具体长度按照 P_num*period计算

#####################38.901 CDL parameter set################
#####################38.901 CDL parameter set################
...
PhiRange = 120  #水平角度范围
'''if Cluster_selected == 'ON'：
    clusterSet=[]
	for clusterIndex in range(0,clusterNum,1):
		if AOD[clusterIndex] >= -PhiRange/2 and AOD[clusterIndex] <= PhiRange/2 and ZOD[clusterIndex] >= 90 - ThitaRange/2 and ZOD[clusterIndex] <= 90 + ThitaRange/2:
...
'''
InitialPhase_VV = np.random.rand(U, S, clusterNum, rayNum)*0
InitialPhase_VH = np.random.rand(U, S, clusterNum, rayNum)*0
InitialPhase_HV = np.random.rand(U, S, clusterNum, rayNum)*0
InitialPhase_HH = np.random.rand(U, S, clusterNum, rayNum)*0
...
#####################Calculate UE Antenna Element Location################
...
###################Calculate BS Antenna Element Location##################
...
###################Calculate Phase compensation due to down tilt##################
PhaseCompensationMatrix = np.exp(-1j*(k_CONST*Md*np.mod(range(0, S), Mp)*np.cos(np.deg2rad(Theta_tilt))))


t = np.array(range(1, P_num+1))*period  		# Time Samples


#######################Channel Generation main module####################
		...
		for	iCluster in range(0,clusterNum):
			...
			if iCluster == 0 and LOS_or_NLOS.lower() == LOS.lower():
				...
				for isubpath in range(0, M):
					...
					F_tx.H*np.exp(1j*k_CONST*rx.T*du.T)*np.exp(1j*k_CONST*tx.T*ds.T)*np.exp(1j*k_CONST*rx.T*V_direction.T*t)
...
#scio.savemat('ChannelData.mat', {'delay': delay, 'H_real_average': H_real_average, 'H_imag_average': H_imag_average, 'H_average': H_average})
...
#######################output to file.asc ####################
#asc_Vdataoutput(P_num, CentralFrequence, SD, CIR_update_Rate, S/Am, U, 'OutputResults45', channel_type, V*3.6, delay, H_average, H_imag_average, H_real_average)
#asc_Hdataoutput(P_num, CentralFrequence, SD, CIR_update_Rate, S/Am, U, 'OutputResults_45', channel_type, V*3.6, delay, H_average, H_imag_average, H_real_average)
